...
2. **문자열 추출 및 변환 (Extract & Convert)**:
   ...
   // turbo
   ```bash
   sh agents/gemini/skills/localization/scripts/localize.sh extract
   ```
...
3. **번역 스크립트 실행 (Translate)**:
   ...
   - **일반 UI**:
     // turbo
     ```bash
     sh agents/gemini/skills/localization/scripts/localize.sh translate
     ```
   - **설정(Settings) UI**:
     // turbo
     ```bash
     sh agents/gemini/skills/localization/scripts/localize.sh translate-settings
     ```

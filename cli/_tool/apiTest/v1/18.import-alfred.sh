...
if [ -f "$ALFDB" ]; then
  RESULT=$(curl -s --connect-timeout 3 -X POST "$BASE/import/alfred" \
    ...
    -d "{\"db_path\":\"$ALFDB\"}")
  SUCCESS=$(echo "$RESULT" | jq -r '.ok // false')
  if [ "$SUCCESS" = "true" ]; then
    ...
    if [ -n "$CODE" ]; then
      echo "{\"ok\":true,\"data\":{\"message\":\"import API 응답 정상 (code: $CODE, 환경 제한으로 실제 import 생략)\"}}"
    else
      echo "$RESULT" | jq .
    fi
  fi
else
  echo '{"ok":true,"data":{"message":"alfdb not found — skip"}}'
fi
